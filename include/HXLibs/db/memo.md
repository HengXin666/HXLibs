...
// api (1)
auto res = db.select<col(&User::name), col(&Cat::name).as<"catName">(), sum<Col(&User::catCnt).as<"catCnt">()>>()
             .from<User>()
             ...
             .groupBy<&Cat::color, Expr>()
             .having<Expr>()
             .orderBy<col(&Cat::id).asc()>()
             .limit</* offset */ db::param<uint64_t>, /* maxCnt */ 50>(offset);

for (auto const& v : res) {
    std::println("UserName: {}, CatName: {}, CatCnt: {}",
        v.at<&User::name>(), v.at<"catName">(), v.at<&User::catCnt>());
