...
#include <HXLibs/log/debug/RaiiLog.hpp> // debug
...
class SerialExecutorPro {
    ...
    struct SerialAwaiter {
        explicit SerialAwaiter(SerialExecutorPro& mtx, coroutine::Task<> const& task)
            : _mtx{mtx}
            , _task{task}
        {}
    ...
    SerialTask start() {
        ...
        [this, &runList]() -> SerialTask {
            co_await start().via(runList);
        }().via(runList).detach();
    }
public:
    explicit SerialExecutorPro(EventLoop& loop)
        : _loop{loop}
    {}
    ...
    coroutine::Task<> serial(coroutine::Task<> const& t) {
        ...
        if (auto& isRun = _runList->isRun; !isRun) {
            isRun = true;
            co_await start().via(_runList->runList);
        }
