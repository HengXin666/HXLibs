...
- [2026-06-27 18:36:43] : 修复 RouterTree 在 `find("**")` 时候 `StrHash` 不正确导致的bug. 新增 RouterTree 的 testCase
...
- [2026-05-31 16:04:59] : 修复 Sqlite3 内存泄漏问题: 子类成员比父类先析构. 导致 close_db 因为存在 stmt 而失败 (stmt 在父类被管理).

- [2026-05-31 14:45:59] : 实现了串行调度器 (生命周期敏感), 修复部分的类的命名错误; 新增 debug/RAII 类, 方便调试

- [2026-05-24 00:38:35] : 支持服务端使用 SSE (初步设计完成, 待严谨测试, 拟订需要协程锁), 为序列化支持 std 时间段 和 时间点

- [2026-05-23 12:16:13] : 重构客户端 API, 现在不需要在类模板中写死超时时间了, 而是下放到函数模板

- [2026-05-20 00:22:58] : 客户端支持 SSE 解析; 初步架构 AI 大模型的协议; 修复 `CHashMap::find` 的索引越界问题

- [2026-04-06 15:46:00] : 修复 MSVC 上的编译错误. 因为 https://godbolt.org/z/zePvbY39f 类似的问题. MSVC bug (可能)

- [2026-04-05 13:28:38] : 修复 Sql模块的bug (Sqlite3 应该使用 close 而不是 free, 这会导致段错误; 以及几个异常未使用 + 判断翻转了); 为 windows支持 vcpkg 安装依赖. 并重新启用其工作流

- [2026-03-28 19:23:39] : 新增 协程 + 线程池 的使用示例; 自述文档新增 orm-sql 模块的介绍

- [2026-03-01 15:34:08] : 修复悬垂引用问题; 完全支持 乱序 bind 参数
...
- [2026-01-27 00:21:26] : 新增 update 重载: 可以指定插入的成员指针

- [2026-01-20 00:06:26] : 支持 update, 并且支持 returning 和返回变更行数

- [2026-01-18 14:36:12] : 新增 insert 重载: 可以指定插入的成员指针; 补充 db::Constraint 的序列化字符串偏特化接口实现.

- [2026-01-17 23:09:37] : 修复 exec 直接 reset 导致 returning 无法获取到数据的bug; 现在分离, 独立一个 reset 接口

- [2026-01-16 23:54:16] : 支持插入数据, 并且返回指定内容 (不一定是主键)

- [2026-01-14 00:48:26] : 支持插入数据. 待支持返回主键、复合主键等

- [2026-01-13 00:45:39] : 实现编译期获取表中主键或者复合主键所在元素在成员中的索引

- [2026-01-12 00:43:50] : 完善 select 查询接口, 支持 limit 绑定参数; 初步架构 insert 接口; 删除无用接口设计. 新增部分 DB 接口 (如新建数据表, 支持 IsNotExists 选项)

- [2026-01-11 18:34:34] : 初步支撑 select 查询. 一些细节接口还没有增加

- [2026-01-10 13:02:25] : 新增 Select 返回类型, 支持 get idx / asName 获取. 修改部分接口定义. 为 FStr 新增 `==` 运算符重载

- [2026-01-08 00:57:07] : 初步 CRTP 架构 Sqlite3 DB, 支持 Dml 保存传入参数 (未完善, 待研究 std::tuple<Ts&&...> 是否 与 {std::forward<Ts>(ts)...} 合法)

- [2026-01-07 00:29:05] : 支持约束db表达式传入的类型, 并且可约束对应位置得对应类型. 目前遇到一个问题, 如何在链式调用中保证仅调用一次(仅初始化时候), 并且可以转发参数.

- [2026-01-05 00:32:06] : 基本架构动态参数绑定. 目前编写了元模板, 可以编译期获取 `db::param` 类型列表 (按照顺序); 发现占位参数在 `in` / `like` 等地方仍没有约束...

- [2026-01-03 23:47:46] : 初步完成SQL CRUD 字符串生成, 还差运行时绑定, 以及对接实际的数据库api

- [2026-01-03 19:57:32] : 重构: Expression 应该传入参数(聚合初始化), 保障_ptr的传递 (编译期)

- [2026-01-03 18:33:28] : 新增可自定义表名; 支持 select/from/join 的sql序列化

- [2026-01-03 15:42:51] : 新增聚合函数, 占位参数; 补充部分表达式运算符重载的约束, 使得类型安全

- [2026-01-02 19:24:00] : 补充部分注释, 新增OrderType作为orderBy模板NTTP, 不混合Expr

- [2026-01-02 19:02:43] : 完成对表达式的架构

- [2025-12-13 21:57:15] : 临时修复 (#17) MSVC ICE, 此为临时解决方案, 因为深层原因无法复现.

- [2025-12-13 18:28:38] : 修改 CustomType api 命名, 并且提供默认支持的 std::opt<> 作为 null 值映射

- [2025-12-13 15:11:43] : 支持定义复合主键, 库决定仅做必要的编译期判断, 复杂的如外键相关的基于复合主键之类的, 判断非常复杂而且浪费编译器算力, 故以运行时异常体现 (创建表时候待数据库报错)

- [2025-12-11 00:26:01] : 支持反射复合索引, 发现应该约束多主键(难), 还有复合主键没有支持

- [2025-12-07 20:44:39] : 新增 编译期字符串NTTP 的 `_fs` 自定义字面量, 尝试修改`IsConstructibleVal`为具体类型, 似乎意外. 重构部分 SQL 建表生成部分, 支持 index 和 外键. 还差联合索引, 修复一些bug.

- [2025-12-06 23:58:18] : 初步实现一个基于反射的 SQL 表创建实现方案, 目前还不支持索引.

- [2025-12-06 12:59:09] : 初步架构 SQL 部分约束. 修改部分 `HX::meta` 的命名.

- [2025-11-29 14:46:36] : 新增 无状态 服务端 支持反射 的 token; 修改cmake, 现在默认是自动查找是否存在 openssl, 如果存在就链接!

- [2025-11-27 00:41:06] : 重构, 基于编译期多态, 分 Http / Https 的服务端, 并且 WebSocket 也区分 WS / WSS; 常态化openssl, 为后续 jwt 提供架构支持.

- [2025-11-07 00:11:13] : 修复潜在的复杂路径映射错误. 修改为使用递归编写. (因为递归几乎不会超过 6 层), 并且修改为零拷贝. 仅传递指针, 而不是需要复制的 `std::optional<Func>`

- [2025-11-04 18:07:57] : 修复 MSVC 编译错误: 原因是 MSVC 编译器太垃圾了! RVO 优化不完全. 还发现协程AOP切面展开失败. 只能使用古法递归展开了...

- [2025-11-03 23:58:48] : 修复玄学bug导致的 GCC 协程bug, 添加了文档链接到自述文档中:
